...
ctx.WriteLocalFile("name", "demo.txt")
ctx.WriteFile("name", []byte("hello world"))
ctx.WriteHTML(http.StatusOK, "")
...
ctx.AddContentType("application/json")
ctx.SetContentDisposition(fmt.Sprintf("attachment; filename=\"%s\"", "hello.txt"))
ctx.AddContentDisposition(fmt.Sprintf("attachment; filename=\"%s\"", "hello.txt"))
